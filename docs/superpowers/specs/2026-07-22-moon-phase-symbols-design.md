...
**Scope:** Add a small, decorative `moonphase.*` SF Symbol next to the moon-phase name at every Timeline site where the phase name renders (five sites) — derived structurally from the stored event, never from displayed text. Follow-up #4 (final queued item) after [[hide-season]]. Purely presentational.
...
**The five sites** (all confirmed against the code; the fifth was found by the pre-execution plan audit — the spec originally said four):
...
5. **Event detail "Details" card** — `EventDetailView.metadataRows` decodes metadata directly (bypassing `EventDisplay`) and renders a `Moon phase | Full Moon` row on the same screen as site 4. Unique to metadata-borne readings — AQI lives in `event.value`, so the AQI round never met this path. Gating is structural here too: the RAW metadata key `"phase"` (a storage key, not the display label) AND the event-level extractor — so a non-moon event that happens to carry `"phase"` metadata (e.g. a cycle phase) renders plain text, never a moon glyph.
...
- **`EventDetailView`** — same extractor-gated branch in the header value line. Additionally (site 5), `metadataRows` keeps the RAW key alongside the display label (`(key, label, value)` triples, `ForEach` id on the raw key — strictly more collision-proof than the current label id), and the shared `row(_:_:)` helper gains an optional `moonPhase` parameter: the details card passes `moonPhaseName(for: displayEvent)` for the `key == "phase"` row and nil otherwise, so the phase row renders `MoonPhaseLabel` while every other metadata row (and the `whenCard` call sites) is unchanged. The card also filters the `"provenance"` metadata key from presentation (Leo, device gate 2026-07-22): it is the engine's internal `TemporalProvenance` flag, not health information. ONLY that key is filtered — stored metadata keeps it (mining depends on it), and other unknown keys stay visible since they may later carry imported or user-facing values.
...
- **Device (visual):** all five sites show the correct glyph per phase (site 5: the detail sheet's Details card "Moon phase" row); light + dark legible; hierarchical rendering reads at footnote size; VoiceOver reads the phase once at every site; text always present. (SwiftUI view code is build + device gate, per the AQI precedent.)
