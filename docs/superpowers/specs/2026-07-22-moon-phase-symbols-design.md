...
- **`EventDetailView`** — same extractor-gated branch in the header value line. Additionally (site 5), `metadataRows` keeps the RAW key alongside the display label (`(key, label, value)` triples, `ForEach` id on the raw key — strictly more collision-proof than the current label id), and the shared `row(_:_:)` helper gains an optional `moonPhase` parameter: the details card passes `moonPhaseName(for: displayEvent)` for the `key == "phase"` row and nil otherwise, so the phase row renders `MoonPhaseLabel` while every other metadata row (and the `whenCard` call sites) is unchanged.
