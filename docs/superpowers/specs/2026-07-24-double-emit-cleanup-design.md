...
Nothing prevents them overlapping. At a cold launch the first two routinely both fire:
...
| 2 | Ordinary duplicates | **Ignored** while a pass runs (`.task` + `.active` at cold launch produce exactly one pass). Joining was the alternative; behaviourally identical because all three call sites are fire-and-forget, so the simpler option wins. |
...
| 6 | Trigger sites | **Unchanged.** Single-flighting makes the trigger count irrelevant, so there is no launch-path behaviour risk. They only change *what they call*. |

## 3. Architecture

### A. `EnvironmentEmitCoordinator` (new, app layer)

A small `@MainActor final class` owning two pieces of state and one method. It does not know about the emitter, the database, or the store — it takes an injectable `perform` closure, so tests drive it with a counting stub and never touch the network or DB.
...
final class EnvironmentEmitCoordinator {
    ...
    func emit(forced: Bool) async {
        if forced { pendingForced = true }
        guard !isRunning else { return }        // a pass is active; it will drain the flag
        isRunning = true
        defer { isRunning = false }

        var runForced = forced
        while true {
            pendingForced = false
            await perform(runForced)
            if !pendingForced { break }
...
- **Dedup:** a second `emit` while `isRunning` returns immediately without starting a pass.
- **Queued recovery:** a `forced` call sets `pendingForced` before the guard, so it is recorded even when it returns early; the running loop drains it after its current `perform`.
...
- **No lost-wakeup window.** Everything is `@MainActor`, and there is no `await` between the final `pendingForced` check, the `break`, and `defer { isRunning = false }`. A trigger therefore cannot slip in after the loop decides to exit but before the flag clears — precisely the gap where a naive implementation drops a heal.
...
The closure captures the service and store; neither holds the coordinator, so there is no retain cycle. The coordinator has no observable state the UI reads, so it is held as plain `@State` rather than `@StateObject` (honest about not being an `ObservableObject`).

The three triggers keep their existing structure and gating and only change their call:

- `.task` → `await coordinator.emit(forced: false)`
- scenePhase `.active` → `await coordinator.emit(forced: false)`
- `locationRecoveryTick` → `await coordinator.emit(forced: true)`, **keeping its existing live-location-failure gate** (that gate is what bounds the trailing-pass loop).
...
All against the injected `perform` closure — no network, no database, no `UserDefaults`. The stub records each call's `forced` value and can be held open (e.g. by awaiting a continuation the test resumes) so a second `emit` provably lands mid-pass.

- **Normal dedup:** two concurrent `emit(forced: false)` → `perform` runs **once**.
...
- **Cleanup:** after everything settles, a later `emit` starts a fresh pass — `isRunning` and `pendingForced` are both reset.
