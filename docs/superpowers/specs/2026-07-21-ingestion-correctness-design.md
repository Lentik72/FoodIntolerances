...
| 3 | AQI window | **Previous completed local calendar day.** `/air_pollution/history` (free) for `[startOfYesterday, startOfToday)` in local time → the **completed local-day mean** PM2.5 → EPA AQI, event **timestamped to yesterday's noon**. Genuinely retrospective; on DST days the window is 23 or 25 hours, not literally 24. |
...
- A **pure static** `dailyMeanPM25(slots:dayStart:dayEnd:minHours:) -> Double?` — mean PM2.5 over `dt ∈ [dayStart, dayEnd)`; requires **≥ `minAirQualityHours = 20`** in-window slots else nil (partial-history → a legitimately absent day). On DST days the window is 23/25h ("completed local-day mean", not literally 24).
- **One range request, not N.** `fetchCompletedAirQualityRange(from:through:) -> AQIRangeResult` makes a SINGLE `/air_pollution/history` call spanning the whole backfill window and groups the hourly slots into local days: `.fetchError` (whole-request network/decode failure → retry the window) or `.days([Date: AQIDayValue])` where each day is `.value(Int)` or `.absent`. (Three-state matters: a decode error must retry, never be mistaken for per-day absence.)
- **Backfill (in the emitter) — contiguous watermark, retry-throttled.** Window `[max(lastAQIDay+1, yesterday − (maxBackfillDays−1)), yesterday]` (= 30 days), fetched in one range request gated by a `lastAQIAttemptAt` + `minAQIRetryInterval` throttle (so a recent-gap tail or repeated failure doesn't re-download the range every foreground). Walk oldest→newest: `.value` → emit + (while contiguous) advance; **recent** = the last `gracePartialDays = 2` completed days (i.e. yesterday and the day before; the cutoff day itself is OLD) — a recent absent day (likely provider lag) stops advancement (retry); an **old** absent day advances (resolved-absent, so a permanent gap can't block forever). Value days beyond a recent gap still emit (dedup-idempotent); only the watermark holds at the first unresolved gap so gaps are retried, never skipped. `.fetchError` advances nothing. `lastAQIDay` is a `Date?`, **`nil` when unset** (backfill then starts at `capFloor`).
...
**Upgrade migration.** Because the env dedup-key format changes, a GRDB migration (raw SQL/`Row`, not the record type — migration bodies are frozen) rewrites **every** `category='environment'` row **including soft-deleted ones, preserving `deletedAt`** (do NOT filter `deletedAt IS NULL`) so new emissions dedup against them — including tombstones, which `IngestPipeline` matches by exact dedup key to honor "user deletes are never resurrected." ("Reset first" is then simulator-only convenience, not a real-data requirement.) Legacy classification is conservative, by subtype: temperature/humidity → `forecast`; pressure/pressureDrop → `currentSnapshot`; moon/season/mercury → `observedCompletedDay`; **legacy airQuality → `forecast` (NEVER observed** — from the forward-looking impl, so the fail-closed gate must not mine it); unknown subtypes → unclassified.

**airQuality tombstone exception.** Because legacy airQuality classifies to `forecast` but the new emitter emits `observedCompletedDay`, a deleted legacy (forecast) AQI does NOT suppress a new observed AQI for that day — a genuinely different, better datum. This is unreachable for real users (the AQI feature is unshipped; legacy AQI exists only on dev builds of this branch), so the never-resurrect guarantee is scoped to non-AQI subtypes.
...
- **Orchestration (the #1 regression):** one `fetchAllData()` with a stub transport answering all three endpoints → `currentPressure` AND `forecastHighC` AND `forecastAQI` are all populated (the observable all-three assertion is the regression coverage; a direct `isCancelled` pin is vacuous because `currentAtmosphericTask` is `private(set)` and nil in a test).
...
- **Partial-history:** history returns `< minAirQualityHours` distinct hours → `dailyMeanPM25` nil → no AQI event for that day; the watermark advances past it only once it's OLD (outside the `gracePartialDays` window) — a recent partial retries (provider-lag grace).
