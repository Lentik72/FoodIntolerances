...
- `fetchCompletedAirQuality(for dayStart:)` — resolves the local day window `[startOfDay(D), startOfDay(D+1))` (DST-correct via the injected calendar), GETs the history URL, aggregates, `epaAQI`, and returns `Int?` (nil on network error OR partial/empty).
