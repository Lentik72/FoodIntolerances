...
- **Backfill (in the emitter) — contiguous watermark.** Window `[max(lastAQIDay+1, yesterday − (maxBackfillDays−1)), yesterday]` (= 30 days). Walk oldest→newest: `.value` → emit + (while still contiguous) advance; an **old** absent day (older than `gracePartialDays = 2` before yesterday) → advance (resolved-absent, so a permanent gap can't block forever); a **recent** absent day (within the grace window — likely provider lag) → stop advancing (retry next foreground). Value days beyond a recent gap still emit (dedup-idempotent); only the watermark holds at the first unresolved gap so gaps are retried, never skipped. `.fetchError` advances nothing. `lastAQIDay` defaults to `.distantPast` unset.
...
**Upgrade migration.** Because the env dedup-key format changes, a GRDB migration rewrites existing `category='environment'` rows so new emissions dedup against them (no duplicate rows on upgrade — "Reset first" is simulator-only convenience, not a real-data requirement). Legacy classification is conservative, by subtype: temperature/humidity → `forecast`; pressure/pressureDrop → `currentSnapshot`; moon/season/mercury → `observedCompletedDay`; **legacy airQuality → `forecast` (NEVER observed** — it came from the forward-looking next-24h implementation, so the fail-closed gate must not mine it); unknown subtypes → left unclassified.
