...
## Post-Review Fixes (found by Leo before the device gate, on top of the reviewed branch)

The whole-branch review passed READY-TO-MERGE, but a pre-device-gate pass caught three P1 gaps. Fixed on-branch (commits `9391cd2` A+B, `5157d7e` C), each re-reviewed clean.

- **P1-A — self-heal could not happen in one pass.** `requestRefreshWithCooldown` has a 5-min cooldown and each backfill a 1-hour retry throttle, and nothing re-ran the environment fetch when Location recovered — so "restore Location, foreground once, clear every marker" was impossible, and a cold-launch fix that resolves seconds after the first fetch had no trigger at all. Fix: `emitIfNeeded(bypassThrottles:)` + `requestRefreshWithCooldown(bypassCooldown:)` skip the cooldown, and the backfills skip their retry-interval guard (watermark still written; grace/contiguity untouched), under bypass only. A **location-recovery trigger**: `EnvironmentalDataService` observes `LocationService.$currentLocation` (`.receive(on:.main)` so `provenance` set in `apply()` is current) and bumps a `@Published locationRecoveryTick` when `resolvedCoordinate() != nil` (a *trusted* coord — a fabricated NYC fix does not bump it). `FoodIntolerancesApp.onChange(of: locationRecoveryTick)` fires `emitIfNeeded(bypassThrottles: true)` **only** when a live `locationDenied`/`locationUnavailable` failure exists — bounding the bypass to real recovery and self-limiting once it heals. Bypass plumbing is unit-tested (both backfills fetch despite recent attempt watermarks; cooldown-bypass); the CoreLocation trigger is device-verified.

- **P1-B — cancellation lost precedence to classification.** In both backfill fetches the `Task.isCancelled` guard ran *after* `httpStatusReason` + decode, so a task cancelled while the transport returned a 401 / malformed JSON / absent body returned `.fetchError(...)`/`.absent` instead of `.cancelled` — violating the total-no-op cancellation contract. Fix: a `Task.isCancelled` check **immediately after `transport.data`, before `httpStatusReason`/decode**, in both fetches (the pre-existing pre-return guard retained as insurance). Tested with a self-cancelling transport returning a 401 → `.cancelled`.

- **P1-C — Health diagnostics omitted *when*.** `Explanation` dropped `EnvironmentFailure.at` (no "Last tried …" line), and "Updated …" was always date-omitted (a days-old update read "Updated 9:14 AM"). Fix: `Explanation` carries `at` (from the same failure that made heading/body — live failure or most-recent `lastFailure`); a pure `timestampStyle(for:now:calendar:) → .timeToday | .dateOlder`; the views render a "Last tried …" line and format "Updated" adaptively (time today / abbreviated date older). The presentation stays format-agnostic (returns `Date` + style; views call `.formatted`).

**Note (supersedes the P1-A "5s timeout" Minor recorded earlier):** the bypass/recovery machinery does not change the internal 5-s pressure timeout-fallback's no-status window; that remains a bounded, self-correcting Minor.
