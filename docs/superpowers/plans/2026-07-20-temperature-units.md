...
```
  ...
  (The `get` resolves an empty stored value to the locale default, so the segment shows the resolved unit on first open rather than blank. Do NOT wrap the row in `.accessibilityElement(children: .combine)` — that would merge the segmented control into one element and can strip its per-segment operability; leave the Picker natively accessible, with the label above.)
...
- [ ] **Step 5: On-device / simulator check** (device preferred):
  ...
  - **VoiceOver:** the Temperature row's segmented control is still individually operable (swipe-adjust / double-tap flips °C↔°F).
