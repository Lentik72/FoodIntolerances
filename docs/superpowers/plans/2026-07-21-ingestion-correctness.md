...
- [ ] **Step 4b: Upgrade migration (write its tests first).**
  - **Classification + key-PARITY test:** seed legacy `.environment` events (no provenance, OLD key) for each subtype **including a non-UTC `timezoneID`** (e.g. `"America/Los_Angeles"`) so a `Calendar.current`-instead-of-row-timezone bug is caught. Run the migrator; assert each row now carries the conservative provenance AND a dedupKey that **exactly equals `DedupKey.daily(.environment, subtype, dayStart:, provenance:)`** computed the same way the factory does (not merely "contains provenance" — parity is the whole point, or duplicates on upgrade): temperature/humidity → `forecast`, pressure/pressureDrop → `currentSnapshot`, moonPhase/season/mercuryRetrograde → `observedCompletedDay`, **legacy airQuality → `forecast`** (assert `temporalProvenance != .observedCompletedDay`, so a legacy forecast-derived AQI is NOT mined by the fail-closed gate).
  ...
  - Register a new migration (idempotent, additive; do NOT edit an already-shipped migration) that rewrites **every** `category='environment'` row — **including soft-deleted ones, preserving `deletedAt`** (do NOT filter `deletedAt IS NULL`). **Use raw SQL + `Row`, NOT the `HealthEvent` record type** (migration bodies are frozen; a future column added to `health_events` would make a record-type-based v6 body crash on fresh install): `SELECT id, subtype, timestamp, timezoneID, metadata FROM health_events WHERE category='environment'`, compute provenance by the subtype table + the new metadata + `DedupKey.daily(...)` (dayStart from `timestamp` in the row's `timezoneID`), then `UPDATE health_events SET metadata=?, dedupKey=? WHERE id=?`. Unknown subtypes → leave unclassified (nil provenance).
