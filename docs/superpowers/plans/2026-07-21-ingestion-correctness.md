...
- **Watermark advances contiguously.** `lastAQIDay` is a `Date?` (**`nil` when unset → the backfill starts directly at `capFloor`**, no `.distantPast` arithmetic). It advances only through consecutive *resolved* days from the bottom; it STOPS at the first unresolved day so gaps are retried, never skipped.
- **AQI range request is retry-throttled.** A separate `lastAQIAttemptAt` + `minAQIRetryInterval` (e.g. 3h) gates the `/air_pollution/history` range GET, so rapid foregrounding cannot re-download the 30-day range while the tail day is a recent gap or the range keeps failing (independent of the pressure/forecast cooldown).
- **Coverage counts DISTINCT hourly timestamps.** `dailyMeanPM25`'s `minAirQualityHours` check (and the mean) count distinct hourly `dt` values, not raw array entries — a provider returning duplicate slots must not satisfy coverage or double-weight the mean. Resolution per day: `.value` → emit + resolved; **old** partial/absent (older than `gracePartialDays = 2` before yesterday) → resolved-absent (advance, don't block forever); **recent** partial/absent (within `gracePartialDays` of yesterday — likely provider lag) → UNRESOLVED (retry next foreground). A whole-range network/decode failure advances nothing (retry). Day stepping uses `calendar.date(byAdding: .day, …)`, NOT `+86_400`. `minAirQualityHours = 20`.
...
- [ ] **Step 4b: Upgrade migration (write its tests first).**
  - **Classification + key-PARITY test:** seed legacy `.environment` events (no provenance, OLD key) for each subtype **including a non-UTC `timezoneID`** (e.g. `"America/Los_Angeles"`) so a `Calendar.current`-instead-of-row-timezone bug is caught. Run the migrator; assert each row now carries the conservative provenance AND that its migrated dedupKey **exactly equals `DedupKey.daily(.environment, subtype, dayStart:, provenance:)`** (parity is the whole point — a mismatch = duplicates on upgrade). This parity test is what pins the frozen migration builder (below) to today's factory format: temperature/humidity → `forecast`, pressure/pressureDrop → `currentSnapshot`, moonPhase/season/mercuryRetrograde → `observedCompletedDay`, **legacy airQuality → `forecast`** (assert `temporalProvenance != .observedCompletedDay`, so a legacy forecast-derived AQI is NOT mined by the fail-closed gate).
  - **Tombstone (never-resurrect) test — REQUIRED, NON-AQI subtype:** seed a **soft-deleted** (`deletedAt != nil`) legacy env event **of a subtype whose legacy and new-emission provenance MATCH** (use `moonPhase` → `observedCompletedDay`, or a `.forecast` temperature); run the migration; `IngestPipeline.ingest(...)` the equivalent NEW-format event; assert **zero VISIBLE rows appear for that key** (query visible rows, not just the insert count — this also guards that the migration preserved `deletedAt`; a rewrite that cleared it would surface a row and fail). `IngestPipeline.process` blocks resurrection by exact-dedupKey match against soft-deleted rows, so the migrated tombstone key must equal the new emission's key.
  - **⚠️ airQuality tombstone is a documented exception (do NOT test AQI here):** legacy `airQuality` classifies to `forecast` while the new emitter emits `observedCompletedDay` → different keys → a new observed AQI is NOT suppressed by a deleted legacy forecast AQI. This is by design (classifying legacy AQI observed would wrongly make it mineable) and unreachable for real users (the AQI feature is unshipped — legacy AQI exists only on dev builds of this branch). A new observed AQI legitimately supersedes a deleted legacy forecast AQI.
  - Register a new migration (idempotent, additive; do NOT edit an already-shipped migration) that rewrites **every** `category='environment'` row — **including soft-deleted ones, preserving `deletedAt`** (do NOT filter `deletedAt IS NULL`). Migration bodies are FROZEN, so it must not depend on any evolvable production type OR helper:
    - **Not the `HealthEvent` record type** (a future column added to `health_events` would crash a record-based v6 on fresh install) — use raw SQL + `Row`.
    - **Not `DedupKey.daily(...)`** — a future migration changing the key format would make v6 emit the NEW format when it reruns on a fresh install. Inline a **migration-local frozen key builder** with the exact format, matching `DedupKey.minute` semantics (`Int(dayStart.timeIntervalSince1970 / 60)`):
...
```
    ...
    - Body: `SELECT id, subtype, timestamp, timezoneID, metadata FROM health_events WHERE category='environment'`; per row compute the conservative provenance + updated metadata + `dayStart` (from `timestamp` in the row's `timezoneID`) + `v6EnvironmentKey(...)`, then `UPDATE health_events SET metadata=?, dedupKey=? WHERE id=?`. Unknown subtypes → leave unclassified (nil provenance). The parity test above (v6 key == `DedupKey.daily(...)`) guarantees v6's frozen format matches today's factory; any future key change gets its OWN migration, never a v6 edit.
...
- [ ] **Step 1: Update the failing tests first** to the 2024 expectations: `epaAQI(9.0)==50`, `9.1==51`, `12.0==56`, `12.1==57`, `6.0==33`, `35.4==100`, `35.5==101`, `55.4==150`, `55.5==151`, `125.4==200`, `125.5==201`, `225.4==300`, `225.5==301`, `325.4==500`, `325.5==500` (top-bin both sides + clamp), `9999==500`. **THREE existing assertions flip and MUST be updated: `epaAQI(12.0)` 50→56, `epaAQI(12.1)` 51→57, `epaAQI(6.0)` 25→33** — they were correct only under the old table; leaving any fails after Step 3. `45.0==124` and `35.49==100` are unchanged. Category names/bands unchanged; keep `poorAirThreshold==101` and `category(101)==.unhealthySensitive`. (Arithmetic: `12.1` → bin (9.1,35.4,51,100): `(49/26.3)*(12.1−9.1)+51 = 56.59 → 57`; `12.0 → 56`; `6.0` → (0,9,0,50): `(50/9)*6 = 33.3 → 33`.)
...
**Files:**
- Modify: `APIConfig.swift` (`airPollutionHistoryURL`), `EnvironmentalDataService.swift` (`completedDayWindow`, `dailyMeanPM25`, `fetchCompletedAirQualityRange`)
...
- [ ] **Step 1: Write the failing tests first.**
  - `dailyMeanPM25` — mean over `dt ∈ [dayStart, dayEnd)`; a mean-CHANGING out-of-window slot is excluded; **boundary: exactly `minAirQualityHours (20)` DISTINCT in-window hours → value, exactly `19` → nil** (pins `>=` not `>`); **half-open: a slot at exactly `dayEnd` is EXCLUDED**; **duplicate slots don't count or double-weight**: 20 in-window slots where two share the same `dt` = 19 distinct hours → nil (a raw-`count`/raw-mean impl would pass 20 and skew the mean — pins distinct-timestamp counting).
  - **Local-day/DST window** — `completedDayWindow(for:calendar:)` returns `[startOfDay(D), startOfDay(D+1))`. With an injected **America/Los_Angeles** calendar, assert the span is **23h on 2025-03-09 (spring-forward)**, **25h on 2025-11-02 (fall-back)**, and rolls correctly across a **month** (D = 2025-01-31) AND a **year** (D = 2025-12-31 → end = 2026-01-01). (A naive `−86_400` or a UTC calendar yields 24h and fails these.)
...
- [ ] **Step 3: Implement.**
  - `APIConfig.airPollutionHistoryURL(latitude:longitude:start:end:)` where `start`/`end` are `TimeInterval` → `\(base)/air_pollution/history?lat=&lon=&start=\(Int(start))&end=\(Int(end))&appid=` (callers pass `window.start.timeIntervalSince1970`, since `completedDayWindow` returns `Date`s).
  ...
  - `static func dailyMeanPM25(slots: [(dt: TimeInterval, pm25: Double)], dayStart: Date, dayEnd: Date, minHours: Int) -> Double?` — filter `dt ∈ [dayStart.tis, dayEnd.tis)`, then **de-duplicate by `dt`** (keep one pm2.5 per distinct hourly timestamp), `guard distinct.count >= minHours`, return the mean over the de-duplicated set.
...
- [ ] **Step 1: Write the failing tests first** (inject clock + calendar + an in-memory `WatermarkStore` + a stub `EnvironmentalDataProviding` whose `fetchCompletedAirQualityRange` returns a scripted `AQIRangeResult` and whose published readings are canned):
  ...
  - **Contiguous watermark stops at a gap (the #1-intent test, pinned dates):** clock → yesterday = 2025-06-10, `gracePartialDays = 2`. Range maps `2025-06-08 → .value`, `2025-06-09 → .absent` (recent), `2025-06-10 → .value`. Assert BOTH `06-08` and `06-10` emit AQI events AND the watermark == **2025-06-08** (NOT `06-10` — it does not jump past the `06-09` gap); the next foreground re-requests from `06-09`. (A `break`-out-of-switch or skip-the-gap bug lands the watermark at `06-10`.)
  - **Grace boundary pinned with named dates** (yesterday = 2025-06-10, recent = {2025-06-09, 2025-06-10}, graceCutoff = 2025-06-08): an absent **2025-06-07** (old) → watermark advances past it, no event; an absent **2025-06-09** (recent) → watermark does NOT advance past it, retried; an absent **2025-06-08** (== cutoff) is treated as OLD (advances) — pins `>` vs `>=`.
  ...
  - **Backfill cap + single request:** unset watermark (`nil`) → exactly ONE `fetchCompletedAirQualityRange` call spanning `[yesterday−29, yesterday]` (30 days); emitted events dated to each day's local noon.
  - **DST-safe stepping (pinned):** injected America/Los_Angeles calendar, clock = 2025-11-03 (so yesterday = 2025-11-02, the 25-hour fall-back day). A 3-day backfill emits AQI for exactly `{2025-10-31, 2025-11-01, 2025-11-02}` — three distinct local days, no repeat/skip. (A `+86_400` step lands at 23:00 and repeats/skips a day across the transition.)
  - **Retry throttle:** a second foreground within `minAQIRetryInterval` of the first (advance the injected clock by less than the interval) makes NO second `fetchCompletedAirQualityRange` call even though the watermark is still behind (a recent-gap tail); after the interval it re-requests.
  - **Provenance-correct emits:** emitted `airQuality` → `.observedCompletedDay`; emitted `temperature`/`humidity` → `.forecast`.
  - **Idempotent re-emit:** two foregrounds (past the throttle) covering the same completed day → one event (dedup).

- [ ] **Step 2: Run to confirm failure.**

- [ ] **Step 3: Rewrite `emitIfNeeded`** — inject the clock, calendar (timezone), a `WatermarkStore` (default `UserDefaults`), and `service: EnvironmentalDataProviding`; do all day math through them. `protocol WatermarkStore { func date(for key: String) -> Date?; func set(_ date: Date, for key: String) }`, with a `UserDefaults`-backed default conformance (store as epoch `Double`, `nil` when absent).
  ...
  - Remove the `lastEmitDayKey` guard. `lastAQIDay` reads via the store as a `Date?` — **`nil` when unset** (no `.distantPast`).
  - **Same timezone everywhere:** the emitter's injected `calendar`, the service's injected `calendar`, and each emitted reading's `timezoneID` MUST carry the same timezone — the service keys `byDay` by `startOfDay(D)` and the emitter looks it up by `startOfDay(D)`, and the factory recomputes the dedup `dayStart` from `reading.timezoneID`; a mismatch misses every lookup / shifts the key off day D. (Production defaults are `.current` throughout; the DST tests inject the same LA calendar into service + emitter + reading.)
  ...
  - **Backfill (single request, contiguous watermark, retry-throttled — NO in-loop `break`):**
...
```swift
    ...
    // Throttle: while the tail day is a recent gap or the range keeps failing, `start` stays
    // ≤ yesterday, so without this a rapid foreground would re-download the 30-day range every
    // time (independent of the pressure/forecast cooldown). Own interval, own attempt watermark.
