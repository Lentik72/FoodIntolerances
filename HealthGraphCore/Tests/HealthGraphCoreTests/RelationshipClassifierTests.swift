...
struct RelationshipClassifierTests {
    ...
    @Test func triggerAtHighRatioAndConfidence() {
        let e = c.classify(stats: stats(ratio: 3, exposures: 10, spanDays: 30), confidence: 0.6, significant: true, stable: true, now: now)
        #expect(e?.type == .possibleTrigger)
        #expect(e?.status == .active)
    }
    @Test func protectiveAtLowRatio() {
        let e = c.classify(stats: stats(ratio: 0.4, exposures: 10, spanDays: 30), confidence: 0.5, significant: true, stable: true, now: now)
        #expect(e?.type == .improves)
    }
    @Test func noEffectAfterLongNullExposure() {
        let e = c.classify(stats: stats(ratio: 1.0, exposures: 25, spanDays: 120), confidence: 0.1, significant: true, stable: true, now: now)
        #expect(e?.type == .noEffect)
        #expect(e?.status == .confirmedNoEffect)
    }
    @Test func weakUndirectedReturnsNil() {
        let e = c.classify(stats: stats(ratio: 1.1, exposures: 8, spanDays: 20), confidence: 0.2, significant: true, stable: true, now: now)
        #expect(e == nil)
    }
    @Test func lowConfidenceTriggerIsCandidate() {
        let e = c.classify(stats: stats(ratio: 2, exposures: 8, spanDays: 20), confidence: 0.32, significant: true, stable: true, now: now)
    ...
    @Test func veryLowConfidenceTriggerIsDecayed() {
        ...
        let e = c.classify(stats: stats(ratio: 2, exposures: 8, spanDays: 20), confidence: 0.2, significant: true, stable: true, now: now)
    ...
    @Test func nonSignificantTriggerIsCappedToCandidate() {
        let e = c.classify(stats: stats(ratio: 3, exposures: 10, spanDays: 30),
                           confidence: 0.6, significant: false, stable: true, now: now)
        #expect(e?.type == .possibleTrigger)
    ...
    @Test func nonSignificantDoesNotResurrectDecayed() {
        ...
        let e = c.classify(stats: stats(ratio: 3, exposures: 10, spanDays: 30),
                           confidence: 0.2, significant: false, stable: true, now: now)
        #expect(e?.status == .decayed)
    }
    @Test func noEffectIgnoresSignificance() {
        let e = c.classify(stats: stats(ratio: 1.0, exposures: 25, spanDays: 120),
                           confidence: 0.1, significant: false, stable: true, now: now)
        #expect(e?.status == .confirmedNoEffect)
    }
    ...
    @Test func significantButWeakProtectiveIsCandidate() {
        ...
        let e = c.classify(stats: stats(ratio: 0.6, exposures: 10, spanDays: 30),
                           confidence: 0.5, significant: true, stable: true, now: now)
        #expect(e?.type == .improves)
        #expect(e?.status == .candidate)
    }
